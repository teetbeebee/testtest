...
//  private int pageLastIndex; // 本页最后一条
   ...
   *          int
   */
  public PageList(List page, int count, int pageIndex, int pageSize)
      throws Exception {
    // 修正page的值
    List pageFixed = page == null ? new ArrayList() : page;
    // 修正count的值
    if (count < 0) {
      throw new BaseException("无效的count值！！！");
    }
    int countFixed = count;
    // 修正pageSize的值
    if (pageSize < 0) {
      throw new BaseException("无效的pageSize值！！！");
    }
    ...
    pageIndexFixed = pageIndex > pageCountFixed ? pageCountFixed : pageIndex;
    // 设置结果
    this.page = pageFixed;
   ...
   *          int
   */
  public PageList(List page, int pageCount, int count, int pageIndex,
      int pageSize) {
    if (page == null) {
      page = new ArrayList();
    }
  ...
  public String getPositionInfo() {
    StringBuffer sb = new StringBuffer();
    // 1/20
    sb.append(info_gap).append(this.getPageIndex()).append("/").append(
        this.getPageCount());
    return sb.toString();
  }
  ...
  public String getNavInfo(String getPageUrl) {
    ...
    // 第一页
    if (!this.isFirstPage()) {
      sb.append("<a title='第一页' href=").append(getPageUrl).append("&").append(
          info_key_pageIndex).append("=1").append(">|<</a> ");
    } else {
      sb.append("|< ");
    }
    // 上一页
    if (this.isPreviousPageAvailable()) {
      sb.append(info_gap).append("<a title='上一页' href=").append(getPageUrl)
          .append("&").append(info_key_pageIndex).append("=").append(
              this.getPrevPageIndex()).append("><<</a> ");
    } else {
      sb.append(info_gap).append("<< ");
    }
    // 下一页
    if (this.isNextPageAvailable()) {
      sb.append(info_gap).append("<a title='下一页' href=").append(getPageUrl)
          .append("&").append(info_key_pageIndex).append("=").append(
              this.getNextPageIndex()).append(">>></a> ");
    } else {
      sb.append(info_gap).append(">> ");
    }
    // 最后一页
    if (!this.isLastPage()) {
      sb.append("<a title='最后一页' href=").append(getPageUrl).append("&").append(
          info_key_pageIndex).append("=").append(this.getPageCount()).append(
          ">>|</a>");
    } else {
      sb.append(">|");
    }
  ...
  public String getInfo(String getPageUrl) {
    StringBuffer sb = new StringBuffer();
    sb.append(this.getDescInfo()).append(this.info_gap).append(
        this.getPositionInfo()).append(this.info_gap).append(
        this.getNavInfo(getPageUrl));
    return sb.toString();
